...
int f(int i,int j)
{
    for(int k=0;k<4;k++)
    {
        ...
        for(int n=1;n<4;n++)
        {
            if(i+n*hang[k]<=19&&i+n*hang[k]>=1&&j+n*lei[k]<=19&&j+n*lei[k]>=1)
            {
                int q = a[i+n*hang[k]][j+n*lei[k]];
                if(q!=a[i][j])
                {
                    flag = 0;
                    break;
        ...
        if(flag==1)
        {
            ...
            if(i+n*hang[k]<=19&&i+n*hang[k]>=1&&j+n*lei[k]<=19&&j+n*lei[k]>=1)
            {
                if(a[i-hang[k]][j-lei[k]]==0)
                    return 109;
            }
            n=4;
            if(i+n*hang[k]<=19&&i+n*hang[k]>=1&&j+n*lei[k]<=19&&j+n*lei[k]>=1)
            {
                if(a[i+4*hang[k]][j+4*lei[k]]==0)
                    return 109;
            }
        }
    }
