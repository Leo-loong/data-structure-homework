...
int main()
{
    ...
    while(~scanf("%d %d",&xi,&zhi))
    {
        ...
        {
            struct multi *front,*R;
            Q = first;
            while(Q!=NULL)
            {
                P = First->next;
                R = (struct multi*)malloc(sizeof(struct multi));
                front = First;
                R->xishu = xi * Q->xishu;
                R->zhishu = zhi + Q->zhishu;
                R->next = NULL;
                while(P != NULL && R->zhishu < P->zhishu)
                {
                    front = P;
                    P=P->next;
                }
                if(P != NULL && R->zhishu == P->zhishu)
                {
                    P->xishu+=R->xishu;
                    free(R);
                }
    ...
    while(First!=NULL)
    {
        printf("%d %d ",First->xishu,First->zhishu);
        First = First->next;
    }
    return 0;
}
